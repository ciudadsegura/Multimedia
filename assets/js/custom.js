jQuery(document).ready(function() {
   ...
   //Menu nivel 3
   jQuery('.leftpanel .nav .sub-parent > a').click(function() {
      
      var coll = jQuery(this).parents('.collapsed').length;
      
      if (!coll) {
         jQuery('.leftpanel .nav .parent-focus').each(function() {
            jQuery(this).find('.sub-children').slideUp('fast');
            jQuery(this).removeClass('sub-parent-focus');
         });
         
         var child = jQuery(this).parent().find('.sub-children');
         if(!child.is(':visible')) {
            child.slideDown('fast');
            if(!child.parent().hasClass('active'))
               child.parent().addClass('sub-parent-focus');
         } else {
            child.slideUp('fast');
            child.parent().removeClass('sub-parent-focus');
         }
      }
      return false;
   });
